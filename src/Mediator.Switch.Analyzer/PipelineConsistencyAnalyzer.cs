...
public class PipelineConsistencyAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void AnalyzeCompilation(CompilationAnalysisContext context)
    {
        ...
        // Collect all concrete pipeline behaviors — skip generated code types
        var allTypes = GetAllTypes(compilation.GlobalNamespace)
            .Where(t => !IsFromGeneratedCode(t))
            .ToList();
    ...
    /// <summary>
    /// Returns true when every source location of <paramref name="type"/> comes from an
    /// auto-generated file (i.e. any file whose first few lines contain the conventional
    /// <c>&lt;auto-generated&gt;</c> marker, or whose path ends with <c>.g.cs</c>).
    /// Types entirely from metadata (no source locations) are not considered generated.
    /// </summary>
    private static bool IsFromGeneratedCode(INamedTypeSymbol type)
    {
        var sourceLocations = type.Locations.Where(loc => loc.SourceTree != null).ToList();
        if (sourceLocations.Count == 0) return false;

        return sourceLocations.All(loc =>
        {
            var tree = loc.SourceTree!;
            // Fast path: check the file extension
            if (tree.FilePath.EndsWith(".g.cs", StringComparison.OrdinalIgnoreCase))
                return true;

            // Fallback: scan the first 10 lines for the <auto-generated> marker
            var text = tree.GetText();
            var lineCount = Math.Min(10, text.Lines.Count);
            for (var i = 0; i < lineCount; i++)
            {
                if (text.Lines[i].ToString().IndexOf("<auto-generated>", StringComparison.OrdinalIgnoreCase) >= 0)
                    return true;
            }
            return false;
        });
